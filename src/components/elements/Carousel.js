...
class Carousel extends React.Component {
  ...
  componentDidMount() {
    this.setState(
      {
        items: [
          ...Array.prototype.slice.call(this.carousel.current.childNodes),
        ],
      },
      () => this.heightFix()
    )
  ...
  goToNext = (stop = false) => {
    let nextItem =
      this.state.activeItem + 1 >= this.props.children.length
        ? 0
  ...
  handleTouchEnd = (e) => {
    ...
    if (
      ...
      return
    e.changedTouches[0].screenX < this.state.touchStartX
      ? this.goToNext(true)
      : this.goToPrev(true)
  }
          ...
          {Children.map(children, (child, n) => {
            return React.cloneElement(child, {
              ...
              className: classNames(
                ...
                this.state.activeItem === n && "is-active"
              ),
            })
          })}
        </div>
